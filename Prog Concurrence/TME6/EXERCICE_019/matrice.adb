...
package body Matrice is
  ...
  begin
    if Matrice1'length(1)/=Matrice2'length(1) or else 
       Matrice1'length(2)/=Matrice2'length(2) then 
  ...
  begin
	if Matrice1'Length(1)/=Matrice2'Length(2) or else 
	   Matrice2'Length(1)/=Matrice1'Length(2) then 
	   raise Tailles_Non_Compatibles;
        end if;
	for I in 1..Matrice1'Length(1) loop
